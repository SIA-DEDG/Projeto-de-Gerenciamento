...
const BAR_COLORS = ['#0052CC', '#00B8D9', '#FF991F', '#00875A', '#6554C0', '#DE350B', '#FFAB00'];
...
export default function DashboardsPage() {
  ...
  const pending = tasks.filter((t) => t.status_group === 'pending').length;
  const inProgress = tasks.filter((t) => t.status_group === 'in_progress').length;
  const done = tasks.filter((t) => t.status_group === 'done').length;
