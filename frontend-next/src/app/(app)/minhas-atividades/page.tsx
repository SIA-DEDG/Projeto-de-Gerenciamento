...
export default function MinhasAtividadesPage() {
  ...
  const myTasks = useMemo(() => {
    ...
    return allTasks.filter((t) => {
      ...
      if (filterProject  && t.project_id !== filterProject) return false;  // bug fix: estava faltando esta dependência
      if (search) {
  ...
  }, [allTasks, currentUser, filterPriority, filterProject, search]);
    ...
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '18px 32px', borderBottom: '1px solid var(--line-1)', flexShrink: 0, background: 'var(--surface)', flexWrap: 'wrap' }}>
        ...
        {(filterPriority || filterProject) && (
          <button
            onClick={() => patchActiveTab({ filterPriority: '', filterProject: '' })}
            className="mono"
            style={{ fontSize: '0.7rem', fontWeight: 500, color: '#034EA2', background: 'none', border: 'none', cursor: 'pointer', letterSpacing: '0.5px' }}
          >
            LIMPAR
          </button>
        )}
