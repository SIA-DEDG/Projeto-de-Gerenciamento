...
export default function MinhasAtividadesPage() {
  ...
  const myTasks = useMemo(() => {
    ...
    return allTasks.filter((t) => {
      // Inclui tarefas onde o usuário é responsável ou co-responsável
      const isResponsible = t.responsible === userName;
      const isCoResponsible = (() => {
        try { return (JSON.parse(t.co_responsibles ?? '[]') as string[]).includes(userName); }
        catch { return false; }
      })();
      if (!isResponsible && !isCoResponsible) return false;
      ...
      if (search) {
        ...
        if (!t.activity.toLowerCase().includes(q) && !t.category?.toLowerCase().includes(q)) return false;
      }
      return true;
    });
  }, [allTasks, currentUser, filterPriority, filterProject, filterDateFrom, filterDateTo, search]);

  // ── Estatísticas do header ────────────────────────────────────────────────

  const todayStr = new Date().toISOString().split('T')[0];
  const boardStats = useMemo(() => {
    const doneCount      = myTasks.filter(t => t.status_group === 'done').length;
    const openCount      = myTasks.length - doneCount;
    const overdueCount   = myTasks.filter(t => t.status_group !== 'done' && t.deadline && t.deadline < todayStr).length;
    const completedPct   = myTasks.length > 0 ? Math.round((doneCount / myTasks.length) * 100) : 0;
    return [
  ...
  const calendarItems: CalendarioItem[] = useMemo(() => myTasks.map((t) => ({
    id: t.id,
    title: t.activity,
  ...
  async function handleMoveSelected(targetGroup: StatusGroup) {
    await moveSelectedTasks(targetGroup);
  }
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '18px 32px', borderBottom: '1px solid var(--line-1)', flexShrink: 0, background: 'var(--surface)', flexWrap: 'wrap' }}>
        ...
        {(filterPriority || filterProject || filterDateFrom || filterDateTo) && (
          <button
            onClick={() => patchActiveTab({ filterPriority: '', filterProject: '', filterDateFrom: '', filterDateTo: '' })}
            className="mono"
        ...
        <div style={{ flex: 1 }} />
        <span className="mono" style={{ fontSize: '0.7rem', color: 'var(--text-3)', letterSpacing: '0.5px' }}>
          {myTasks.length} ATIVIDADES
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <TaskListView
          ...
          emptyMessage="Nenhuma atividade atribuída a você."
        />
      ) : (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', alignItems: 'start', flex: 1, overflowY: 'auto', minHeight: 0 }}>
            {KANBAN_COLUMNS.map((col) => (
              <KanbanColumn
                key={col.id}
                column={col}
                tasks={myTasks.filter((t) => t.status_group === col.id)}
                projects={projects}
      ...
      {selectionColumn && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '8px 12px', boxShadow: '0 4px 20px rgba(0,0,0,0.12)', zIndex: 200, whiteSpace: 'nowrap' }}>
          <span className="mono" style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-2)' }}>
